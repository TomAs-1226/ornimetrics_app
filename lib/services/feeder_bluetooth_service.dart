...
class FeederBluetoothService {
  ...
  /// Request the runtime Bluetooth permissions (and location on older Android),
  /// then make sure the adapter is on. Without the BLUETOOTH_SCAN/CONNECT grant
  /// on Android 12+, scanning fails instantly.
    ...
    try {
      if (Platform.isAndroid || Platform.isIOS) {
        final statuses = await [
          ...
          // Needed for BLE scanning on Android 11 and below.
          Permission.locationWhenInUse,
        ].request();
        final scan = statuses[Permission.bluetoothScan];
        final connect = statuses[Permission.bluetoothConnect];
        // On older Android the bluetooth* permissions report as granted/n-a;
        // treat an explicit denial of scan or connect as a failure.
        if (scan == PermissionStatus.permanentlyDenied ||
            connect == PermissionStatus.permanentlyDenied) {
          errorMessage.value =
              'Bluetooth permission is off. Enable it in Settings to find feeders.';
          return false;
        }
      }
      if (Platform.isAndroid) {
        try {
          ...
          // User may decline turning BT on; isBluetoothAvailable() reflects it.
        }
      }
    ...
    try {
      ...
      FlutterBluePlus.scanResults.listen((results) {
        ...
        for (final result in results) {
          ...
          // Match both the provisioning device ("Ornimetrics-<x>") and the
          // real on-device BLE status peripheral (named exactly "Ornimetrics").
          if (name.startsWith(kDeviceNamePrefix) || name == 'Ornimetrics') {
  ...
  /// Persist a device added outside the BLE flow (e.g. manual IP setup) and
  /// make it the active feeder.
  Future<void> saveAndSetCurrent(PairedFeeder device) async {
